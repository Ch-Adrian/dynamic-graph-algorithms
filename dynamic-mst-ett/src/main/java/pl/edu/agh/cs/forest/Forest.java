...
public class Forest implements Serializable {
    ...
    private final Map<Integer, Set<Edge>> nonTreeEdges;
    ...
    public Map<Integer, Set<Edge>> getNonTreeEdges(){ return nonTreeEdges; }
    ...
    private void showNonTreeEdges(){
        ...
        for(Integer u : this.nonTreeEdges.keySet()){
            ...
            for(Edge v : this.nonTreeEdges.get(u)){
    ...
    public void addNonTreeEdge(Integer u, Integer v, Integer weight){
        ...
        this.nonTreeEdges.get(u).add(new Edge(u, v, weight));
        this.nonTreeEdges.get(v).add(new Edge(v, u, weight));
    }

    public void deleteNonTreeEdge(Integer u, Integer v) {
        if(u.equals(v)) return;
        if(!this.nonTreeEdges.containsKey(u) || !this.nonTreeEdges.get(u).contains(new Edge(u, v, null)))
            return;
        if(!this.nonTreeEdges.containsKey(v) || !this.nonTreeEdges.get(v).contains(new Edge(v, u, null)))
            return;

        this.nonTreeEdges.get(u).remove(new Edge(u, v, null));
        this.nonTreeEdges.get(v).remove(new Edge(v, u, null));
    }

    public Set<Edge> getNonTreeEdges(Integer u){
        if(!this.nonTreeEdges.containsKey(u))
            this.nonTreeEdges.put(u, new HashSet<>());
    ...
    public void findReplacementEdge(Integer v, Integer w, Integer level) {
        ...
        for(Edge edge: getNonTreeEdgesInOrder(Tmin, level)){
            Integer vertex = edge.getBegin();
            Integer nonTreeEdgeEnd = edge.getEnd();
            ...
            if(!getRepresentativeTreeNode(nonTreeEdgeEnd).get().equals(splayTree.getRootNode(Tmin).get())){
                ...
                if(!hierarchicalForests.get(level+1).checkIfNonTreeEdgeExists(vertex, nonTreeEdgeEnd))
                    hierarchicalForests.get(level+1).addNonTreeEdge(vertex, nonTreeEdgeEnd, weight);
            }
        }
        if(nonTreeEdgeFound){
    ...
    public List<Edge> getNonTreeEdgesInOrder(Node Tmin, Integer level){
        ArrayList<Edge> edges = new ArrayList<>();

        for(Integer vertex: EulerTourTree.getVertices(Tmin)){
            edges.addAll(this.hierarchicalForests.get(level).getNonTreeEdges(vertex));
        }
    ...
    public boolean checkIfNonTreeEdgeExists(Integer u, Integer v){
        if(this.nonTreeEdges.containsKey(u))
            return this.nonTreeEdges.get(u).contains(new Edge(u, v, null));
        return false;
    }
    ...
    public boolean checkIfTreeEdgeExists(Integer v, Integer u){
        ...
        if(this.keyToNodes.containsKey(new Pair<>(v,u))) {
            exists = !this.keyToNodes.get(new Pair<>(v, u)).isEmpty() &&
                    nodeU.isPresent() &&
                    nodeV.isPresent() &&
                    Objects.equals(nodeU.get(), nodeV.get());
        }
        if(this.keyToNodes.containsKey(new Pair<>(u,v)) && !exists) {
            exists = !this.keyToNodes.get(new Pair<>(u, v)).isEmpty() &&
                    nodeU.isPresent() &&
                    nodeV.isPresent() &&
                    Objects.equals(nodeU.get(), nodeV.get());
        }
